...
RT->Config->Get('ExternalSettings')->{My_LDAP}{priv_group_attr} = 'memberUid';
RT->Config->Get('ExternalSettings')->{My_LDAP}{priv_group_attr_value} = 'uid';
diag "test uri login";
{
...
RT->Config->Get('ExternalSettings')->{My_LDAP}{priv_group_scope} = 'sub';
diag "test uri login";
{
    ok( !$m->login( 'testuser1', 'password' ), 'not logged in with real user not in group' );
    $m->warning_like(qr/FAILED LOGIN for testuser1/);
